...
const AllFeedStockRecords = () => {
  ...
  const [feedOverview, setFeedOverview] = useState({}) as any;
  ...
  const getPaginationRange = () => {
    const delta = 1;
    const total = feedOverview?.totalPages || 1;
  ...
  const fetchSortData = () => {
    axios
      .get(
        `${API_URI}/api/dashboard/feed-stock/all-records/${page}?sortBy=${sortByValue}`,
        { withCredentials: true }
      )
      .then((res) => {
        setFeedOverview(res?.data?.allStockData)
  ...
  const fetchFilterData = () => {
    axios
      .get(
        `${API_URI}/api/dashboard/feed-stock/all-records/${page}?filter=${filterValue}`,
        { withCredentials: true }
      )
      .then((res) => {
        setFeedOverview(res?.data?.allStockData)
  ...
  const fetchSearchedData = () => {
    axios
      .get(
        `${API_URI}/api/dashboard/feed-stock/all-records/${page}?search=${searchValue}`,
        { withCredentials: true }
      )
      .then((res) => {
        setFeedOverview(res?.data?.allStockData)
  ...
  return (
    <div className="relative rounded-[20px] bg-white py-6 mx-4 my-4 overflow-hidden">
      ...
      <div className="flex justify-between flex-col gap-2 sm:flex-row items-center mt-3 mx-4">
        <p className="text-sm">
          Showing 1 to 25 of {feedOverview?.totalRecordsCount} entries
        </p>
        <div className="flex items-center gap-5">
          ...
          <button
            onClick={() => setPage(page + 1)}
            className="cursor-pointer border-para rounded-[8px] border px-2 py-1 disabled:cursor-auto"
            disabled={page === feedOverview?.totalPages}
          >
...
export default AllFeedStockRecords;
